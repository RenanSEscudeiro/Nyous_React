...
const CrudEventos = () => {
    ...
    const uploadFile = (event) => {
        ...
        formdata.append('arquivos', event.target.files[0]);
    ...
    const editar = (event) => {
        ...
        fetch(url + '/eventos/' + event.target.value, {
            method : 'GET',
            headers : {
                'authorization' : 'Bearer' + localStorage.getItem('toke-nyous-tarde')
            }
        })
        .then(response => response.json())
        .then(dados => {
                                    ...
                                    eventos.map((item, index) => {
                                        return (
                                            <tr key ={index}>
                                                ...
                                                <td>{item.nome}</td>
                                                <td><a href={item.link} target="_blank">Ir oara o evento</a></td>
